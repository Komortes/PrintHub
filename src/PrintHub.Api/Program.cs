...
using PrintHub.Api.Diagnostics;
...
builder.Services
    .AddOptions<PrintHubFileLoggerOptions>()
    .Bind(builder.Configuration.GetSection(PrintHubFileLoggerOptions.SectionName));
builder.Services
    ...
    CancellationToken cancellationToken) =>
{
    ...
    return TypedResults.Ok(PrintHubDiagnosticsFormatter.ToDto(diagnostics));
})
    ...
    CancellationToken cancellationToken) =>
{
    ...
    var report = PrintHubDiagnosticsFormatter.BuildReport(
        settings,
        queueStatus,
        jobs,
        backendDiagnostics,
...
protectedApi.MapGet("/diagnostics/support-bundle", async (
    IPrintHubSupportBundleService supportBundleService,
    CancellationToken cancellationToken) =>
{
    var bundle = await supportBundleService.CreateAsync(cancellationToken);
    return TypedResults.File(bundle.Content, bundle.ContentType, bundle.FileName);
})
